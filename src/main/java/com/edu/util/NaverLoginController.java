...
public class NaverLoginController {
	...
	public String getAuthorizationUrl(HttpSession session) {
		...
		String url = oauthService.getAuthorizationUrl().replaceAll("\\R", "");
		return url;
		// 위 리턴값으로 네아로의 인증 URL(RestAPI) 생성
	}
	...
	public OAuth2AccessToken getAccessToken(HttpSession session, String code, String state) throws IOException {
		// 네이버 인증 RestAPI에서 인증데이터인 토큰가져오기
		// (동작전) : 현재 컨트롤러에서 발생된 세션의 state값과 비교 -> (동작후) : 콜백 URL로 전달받은 세션검증용 난수값과 네이버로그인 로직에서 
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException {
		// 위 인증데이터인 토큰값으로 네입에서 프로필내용 가져오기
		OAuth20Service oauthService = new ServiceBuilder()
				.apiKey(CLIENT_ID)
				.apiSecret(CLIENT_SECRET)
				.callback(REDIRECT_URL)
		...
		Response response = request.send(); // 프로필을 가져오는 객체를 실행
