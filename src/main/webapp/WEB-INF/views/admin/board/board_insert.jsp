          ...
          <form name="form_write" method="post" action="/admin/board/board_insert" enctype="multipart/form-data">
            <div class="card-body">
              <div class="form-group" style="display:none;">
                ...
                <!-- 세션값을 비교값으로 사용하는 이유는 신규등록이기 때문에 기존게시물정보가 없습니다. -->
...
<script>
	$(document).ready(function(){
		//$('#content_lbl').summernote();//기본실행. 이 기본실행을 개발자 커스터마이징 합니다.
		$('#content').summernote({
			height:150,
			lang:'ko-KR',
		...
		// 서머노트에서 html5의 required의 속성이 작동안되기 때문에 아래코드 추가
		$("form[name='form_write']").on('submit', function(event){
			if ($('#content').summernote('isEmpty')){
				alert('내용은 반드시 입력해주세요');
				event.preventDefault(); // submit 전송기능을 사용금지
			};
		});	
	});
</script>