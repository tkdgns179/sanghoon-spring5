          ...
          <form name="form_write" action="/admin/member/member_insert" method="post" enctype="multipart/form-data">
            <div class="card-body">
              ...
              <div class="form-group">
                <label for="file0">사용자프로필</label>
                <div class="input-group">
                  <div class="custom-file">
                    <input accept=".png" name="file" type="file" class="custom-file-input" id="file0">
                    <label class="custom-file-label" for="file0">파일선택(*png 이미지만 가능)</label>
                  </div>
                </div>
                <div class="mb-2"></div>
              ...
              <div class="form-group">
              	<!-- 신규등록시 ID 중복체크 필수 : button onchange 이벤트 방식이 있음 여기서는  버튼이벤트로 처리함-->	
                <label for="user_id">사용자ID</label>
                <button type="button" id="btn_id_check" class="btn btn-sm btn-secondary">중복체크</button>
...
<script>
$(document).ready(function(){
	// RestAPI서버클래스  맛보기, ID중복체크 메소드를 확인합니다
	// RestAPI Client 맛보기
	$("#btn_id_check").click(function(){
		...
		$.ajax({
			...
			dataType : "text", // 결과값을 받을 떄, 데이터형을 text, json, xml중 선택
			success:function(result){
				alert(result);
				if(result == 0){ // result값이 존재하지 않음 정상진행 
					// 등록버튼을 활성화
					$("#btn_insert").attr("disabled", false)
					alert("사용가능한 아이디 입니다")
				}
				if(result == 1){ // result값이 존재 진행중지
					$("btn_insert").attr("disabled", true)
					alert("올바르지 않거나, 중복된 ID가 존재합니다. 다시 입력해 주세요!")
				}
			},
			error:function(){
