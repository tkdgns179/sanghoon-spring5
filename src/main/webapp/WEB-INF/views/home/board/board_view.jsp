                ...
                <div class="timeline">
                    ...
                    <div class="time-label">
                        <span class="bg-red" data-toggle="collapse" href="#collapseReply" role="button" id="btn_reply_list">
                            댓글리스트[<span id="reply_count">${empty boardVO.reply_count? '0':boardVO.reply_count}</span>]
                        </span>
                    </div>
                    <!-- collapse START -->
	...
	$("#btn_reply_write").click(function(){
		...
		$.ajax({
			...
			complete:function() {
				...
				// alert("등록이 완료 되었습니다.출력3");
				$("#reply_text").val(""); // 입력창에 있는 기존 댓글내용을 지움
				alert("등록이 완료 되었습니다.")
			}
			// Async Javascript And XML(ajax : 비동기 JS & XML) 비동기 false하는 경우 대용량첨부파일 업로드할 떄 
			// success : 업로딩중.. 표시가 됨 -> 업로드가 완료되면 complete가 실행이됨
